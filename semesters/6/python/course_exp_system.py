...
class CarDiagnosticExpertSystem:
    def __init__(self, root):
        ...
        btn_frame.pack()
        self.yes_btn = ttk.Button(btn_frame, text="Да", command=lambda: self.next_state("Да"), bootstyle=PRIMARY)
        self.yes_btn.pack(side=tk.LEFT, padx=20)
        self.no_btn = ttk.Button(btn_frame, text="Нет", command=lambda: self.next_state("Нет"), bootstyle=DANGER)
        self.no_btn.pack(side=tk.LEFT, padx=20)
        self.help_btn = ttk.Button(self.root, text="Помощь", command=self.show_help, bootstyle=INFO)
    ...
    def show_help(self):
        text = self.explanations.get(self.state, "")
        messagebox.showinfo("Пояснение", text)
    
    def show_result(self):
        q, a = self.responses[-1]
        result_title, result_desc = self.results.get(self.state, ("Неизвестная проблема", ""))
        messagebox.showinfo("Результат диагностики", f"{result_title}\n\n{result_desc}")
        self.root.destroy()
