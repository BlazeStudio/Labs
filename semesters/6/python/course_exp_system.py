...
class CarDiagnosticExpertSystem:
    ...
    def load_data(self):
        ...
        # Результаты терминальных состояний
        self.results = {
            31: ("Разряжен или неисправен аккумулятор", "Питание не обеспечивает старт. При напряжении менее 12 В затруднён запуск двигателя. Необходимо проверить уровень заряда, состояние клемм и при необходимости заменить АКБ."),
            32: ("Неисправен стартер/втягивающее реле", "Щелчки без вращения стартера свидетельствуют о его неисправности или плохих контактах. Необходимо протестировать стартер и втягивающее реле."),
            33: ("Отсутствует искра в системе зажигания", "Отсутствие искры мешает воспламенению смеси. Проверяются свечи, катушки, проводка, а также сигналы ЭБУ."),
            34: ("Нет подачи топлива в двигатель", "При отсутствии звука бензонасоса возможно отключение питания или поломка. Проверяется насос, реле, предохранители и разъёмы."),
            35: ("Слишком низкая компрессия", "Нарушение герметичности цилиндров (клапаны, поршни, ГРМ) приводит к невозможности запуска. Требуется замер компрессии и дефектовка."),
            36: ("Ошибка управления двигателем (DTC-коды)", "Коды ошибок, связанные с датчиками и исполнительными механизмами, могут блокировать запуск. Чтение и устранение ошибок — обязательный шаг."),
            37: ("Загрязнённый или неисправный датчик MAF", "MAF измеряет воздушный поток, искажения приводят к неправильной смеси. Возможна очистка или замена датчика."),
            38: ("Повреждённый датчик коленвала", "Отсутствие сигнала от ДПКВ делает невозможной синхронизацию системы зажигания. Необходимо проверить датчик и его разъёмы."),
            39: ("Сбои в системе зажигания при нагрузке", "Коды P030X указывают на пропуски в цилиндрах. Проверяются свечи, катушки, провода, давление топлива и компрессия."),
            40: ("Система исправна — временная или нестабильная ошибка", "Диагностика не выявила конкретной неисправности. Возможна временная ошибка. Рекомендуется повторная проверка при проявлении симптомов.")
        }
